...
**[Open the live page](https://codexguy.github.io/bicharts-react-demo/)** — served from GitHub
Pages, and it makes zero calls to us: it runs on `d3` and `@bicharts/chart-host` alone.
· [RegiaBI for developers](https://bizintelligencechampions.com/regiabi/developers?loc=ghdemo)

A plain Vite + React app rendering two charts generated by RegiaBI — proof that the charts are
not tied to Power BI.
...
Every push to `master` rebuilds the page and publishes it to GitHub Pages
(`.github/workflows/pages.yml`); the workflow sets `DEMO_BASE` so the build is served under
`/bicharts-react-demo/`.
...
- **`@bicharts/chart-host` is an ordinary npm dependency** (a caret range), resolved from the public
...
## A fix made by hand, because the chart is yours

The map's truncation note used to read *"raise Max Map Points (Format > Data)"* — Power BI's
Format pane, which doesn't exist here. The model didn't invent that wording: the generator's own
instructions prescribed it, so the advice was wrong by construction in any host but Power BI.
The generator has since been fixed, and new charts say *"raise the map-point cap to see more"*.

This chart predates that fix, and a generated chart is source you own, so it doesn't change
under you. It was fixed the way you'd fix any other code: one string, edited by hand, in
`src/charts/na-bubbles/chart.js`. Regenerating would have cleared it too, at the cost of
re-rolling the chart's content.
